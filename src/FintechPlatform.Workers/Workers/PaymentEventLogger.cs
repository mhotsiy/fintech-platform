...
/// <summary>
/// Worker that logs payment events for monitoring/debugging
/// </summary>
public class PaymentEventLogger : BackgroundService
{
    private readonly ILogger<PaymentEventLogger> _logger;
    private readonly IConsumer<string, string> _consumer;

    public PaymentEventLogger(ILogger<PaymentEventLogger> logger, IConfiguration configuration)
    {
        _logger = logger;
        ...
        var config = new ConsumerConfig
        {
            ...
            GroupId = "fintechplatform-payment-logger",
    ...
    {
        _logger.LogInformation("Payment event logger starting");
            ...
            while (!stoppingToken.IsCancellationRequested)
            {
                try
                {
                    ...
                    if (result?.Message != null)
                    {
                        ...
                        _logger.LogInformation("📋 Payment Event: {EventType} - {Message}", eventTypeName, result.Message.Value);
        ...
        {
            _logger.LogInformation("Payment event logger stopping");
        }
        finally
        {
            _consumer.Close();
